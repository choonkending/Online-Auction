...
/**
		...
		5. Send Email using mail() function and set bounceback to 2040301@student.swin.edu.au
...
date_default_timezone_set('Australia/Melbourne');
	...
	if( isset($_POST['fname']) && isset($_POST['sname']) && isset($_POST['email']) ){
		...
		if(!empty($_POST['fname']) && !empty($_POST['sname']) && !empty($_POST['email']) ){
			...
			$genPassword = str_shuffle($genPassword);
			...
			if( file_exists($title) ){
				...
				if($found){
					...
					if(preg_match("/^[a-zA-Z0-9!#$%&'*+-=?\/^_`{|}~]+@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/",$email)){ 
						...
						$sent = mail($email,"Welcome to ShopOnline","Dear $fName, welcome to use shopOnline! Your customer id is $genID and the password is $genPassword","From registration@shoponline.com.au", "-r 2040301@student.swin.edu.au");
